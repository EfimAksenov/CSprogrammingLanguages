...
class VerticalLine < GeometryValue
  ...
  def intersectPoint p
    if real_close(x, p.x) then p else NoPoints.new end
  end
  ...
  def intersectVerticalLine vl
    if real_close(x, vl.x) then vl else NoPoints.new end
  end
...
class LineSegment < GeometryValue
  ...
  def intersectWithSegmentAsLineResult seg
    if real_close(seg.x1, seg.x2)
        inOrder = if seg.y1 < y1 then [seg, self] else [self, seg] end
        if real_close(inOrder[0].y2, inOrder[1].y1)
            Point.new(inOrder[1].x1, inOrder[1].y1)    
        elsif inOrder[0].y2 < inOrder[1].y1
            NoPoints.new
        elsif inOrder[0].y2 > inOrder[1].y2
            inOrder[1]
        else 
            LineSegment.new(inOrder[1].x1, inOrder[1].y1, inOrder[0].x2, inOrder[0].y2)
        end
    else
        inOrder = if seg.x1 < x1 then [seg, self] else [self, seg] end
        if real_close(inOrder[0].x2, inOrder[1].x1)
            Point.new(inOrder[1].x1, inOrder[1].y1)    
        elsif inOrder[0].x2 < inOrder[1].x1
            NoPoints.new
        elsif inOrder[0].x2 > inOrder[1].x2
            inOrder[1]
        else 
            LineSegment.new(inOrder[1].x1, inOrder[1].y1, inOrder[0].x2, inOrder[0].y2)
        end
